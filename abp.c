...
void A_output(struct msg message)
{
    ...
    {
        printf("A_output: wait for ack, ignoring the packet: %s\n", message.data);
    }
    else
    {
...
void A_input(struct pkt packet)
{
    if (A.state != WAIT_ACK)
    {
        printf("A_input: A->B unidirection only, ignoring the packet.\n");
    }
    else if (packet.checksum != get_checksum(&packet))
    {
        printf("A_input: packet is corrupted, ignoring the packet.\n");
    }
    else if (packet.acknum < 0) //NAK
    {
        printf("A_input: received NAK.\n");
        stoptimer(0);
        A_resend(); //re-send the packet in response to NAK immediately
    }
    else if (packet.acknum != A.seq)
    {
        printf("A_input: ACK # does not match, ignoring the packet.\n");
    }
    else
    {
        printf("A_input: ACK\n");
...
void A_timerinterrupt(void)
{
    if (A.state != WAIT_ACK)
    {
        printf("A_timerinterrupt: not in waiting for ACK state, ignoring the event\n");
    }
    else
    {
...
void A_resend(void)
{
    if (A.state != WAIT_ACK)
    {
        printf("A_resend: not in waiting for ACK state, ignoring the event\n");
    }
    else
    {
        printf("A_resend: resend last packet: %s.\n", A.last_packet.payload);
        tolayer3(0, A.last_packet);
        starttimer(0, A.estimated_rtt);
    }
}
...
void B_input(struct pkt packet) //wait for ACK#
{   //rdt_rcv(rcvpkt) && (corrupt(rcvpkt) || isNAK(rcvpkt))
    if (packet.checksum != get_checksum(&packet)) //Corrupted, Return NAK
    {
    ...
    {
        printf("  B_input: not the expected seq. send ACK with the previous sequence number.\n");
        send_ack(1, B.last_seq);
    }
    else if (packet.acknum < 0)
    {
        printf(" B_input: receive NAK");
        send_ack(1, B.last_seq);
    }
    else{   //Pass
        ...
        send_ack(1, B.seq);
        B.last_seq = B.seq;
        tolayer5(1, packet.payload);    //print received data
        B.seq = B.seq % 2 + 1; // 1->2, 2->1
    }
}
...
{
    printf("  B_timerinterrupt: No timer.\n");
}
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (TRACE >= 2)
        {
            ...
            {
                printf(" entity: sender A\n");
            }
            else
            {
                printf(" entity: receiver B\n");
            }
        }
